...
- **2026-09-08:** GST Portal offline upload validation root cause: Portal upload validator strictly checks for offline tool signature (`"version": "GST3.2.4"`, `"hash": "hash"`) and rejects files containing portal-download audit fields (`"filing_typ"`, `"cfs"`, `"flag"`, `"updby"`, `"cflag"`, `"chksum"`). Missing `"version": "GST3.2.4"` triggers generic "File could not be uploaded! Download the latest version of Offline tool...".
- **2026-09-08:** Official `GST Offline Tool.exe` reverse-engineering on Linux: Installer unpacked via `innoextract` revealed an Electron/Node app; runnable on Linux directly or headlessly via Bun/Node. Its `service/offline2.js:readXML` parser expects multi-sheet `.xlsx` with a `b2b` sheet and skips 3 header rows, causing single-sheet flat CSVs (`Sheet1`) to be silently dropped.
- **2026-09-08:** GSTR-1 item numbering and inactive tax heads: Official tool sets item `num` to `int(rt * 100)` (e.g. `1800` or `1801`, `500`) and prunes inactive zero-tax heads (`iamt` on intra-state, `camt`/`samt` on inter-state, `csamt` when 0) before running `omitEmpty` to strip empty tables (`b2ba`, `b2cl`, etc.).
- **2026-09-08:** Runtime preference: User strictly prefers Bun (`/usr/bin/bun`, v1.4.0) over Node.js for JS/TS scripts, headless offline tool pipelines, and JSON validations.
- **2026-09-08:** CA-review hardening: corrected gross/reversal and import/ISD routing, held ambiguous/value/head/date matches, separated RCM liability/payment-dependent credit, forwarded filing dates, added shared input contracts, atomic publication, source/rule/output fingerprints and local review/verification commands. Corrected older tests that asserted unsafe accounting; see HARDENING.md for exact scope and remaining limits. Do not claim a measured accuracy percentage or software-generated CA certification.
